...
// Module: Sidebar
// Depends on: next/link, lucide-react, @/lib/utils/icons, @/lib/hooks/usePrefetch, ./index
// Description: Left-side service navigation với prefetch on hover.
// Khi user hover vào sidebar link → prefetch service data trước khi click
// → khi navigate vào trang đó, data đã có sẵn trong ClientCache.
...
import { prefetchServiceOnHover } from '@/lib/hooks/usePrefetch'
import type { SidebarProps } from './index'

/** Renders the primary sidebar navigation with prefetch-on-hover. */
  ...
  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-64'} hidden border-r border-slate-800 bg-slate-900 transition-all duration-200 md:flex md:flex-col`}>
        ...
        {services.map((service) => {
          ...
          return (
            <Link
              ...
              onMouseEnter={() => {
                // Prefetch on hover — data sẽ sẵn sàng trước khi user click
                prefetchServiceOnHover(service.type)
              }}
            >
      ...
      <div className="border-t border-slate-800 p-3">
        <Link
          href="/dashboard/logs"
          className={`flex items-center gap-3 rounded-xl px-3 py-2 text-sm ${currentPath.startsWith('/dashboard/logs') ? 'bg-sky-500/10 text-sky-400' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100'}`}
        >
